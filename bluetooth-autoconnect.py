...
def connect_with_bluetoothctl(device_path):
    try:
        # bluetoothctl'yi başlat
        child = pexpect.spawn('bluetoothctl', echo=False)
        child.sendline(f'connect {device_path}')
        child.expect('Connection successful', timeout=10)
        print(f"successfully connected to device {device_path}", flush=True)
    except pexpect.exceptions.TIMEOUT:
        print(f"error connecting to device {device_path}: timeout", file=sys.stderr, flush=True)
    except Exception as e:
        print(f"error connecting to device {device_path}: {e}", file=sys.stderr, flush=True)
...
def connect_devices_for_adapter(adapter_path):
    ...
    def remove_pending_connection(device_path):
        if not daemon:
            ...
            # If this was the last pending connection, terminate the mainloop
    ...
    for device_path in device_paths:
        ...
        try:
            ...
            if props.get('Connected', False):
                ...
                # Attempt to connect to the device using bluetoothctl
                connect_with_bluetoothctl(device_path)
                remove_pending_connection(device_path)
...
def main():
    ...
    for o, v in opts:
        if o in ('-d', '--daemon'):
            ...
            # This shouldn't ever happen unless we forget to handle an option we've added
            print(f'{SCRIPT_NAME}: internal error: unhandled option {o}', file=sys.stderr, flush=True)
            sys.exit(1)

    # Process commands
    # This script does not use any commands so we will exit if one is incorrectly provided
    if len(cmds) > 0:
    ...
    # This allows commands like `killall SCRIPT_NAME` to function correctly
    try:
        import prctl
        if verbose:
    ...
    if daemon:
        ...
        while True:
            connect_devices_for_all_adapters()
            if verbose:
                print("Waiting for 15 seconds", flush=True)
            time.sleep(15)
        ...
        # If we're waiting for connection attempts to finish, start the mainloop. 
        # We will automatically exit the loop once everything is finished
        if len(pending_connections) > 0:
            loop.run()

def signal_handler(sig, frame):
    if sig == signal.SIGHUP:
        # Rescan adapters and attempt to connect to devices if we're in daemon mode
        if daemon:
            connect_devices_for_all_adapters()
        ...
        # This shouldn't ever happen unless we forget to handle a signal we've added
